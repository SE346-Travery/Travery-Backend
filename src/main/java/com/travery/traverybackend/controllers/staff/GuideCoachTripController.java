...
public class GuideCoachTripController extends AbstractBaseController {
  ...
  public ResponseEntity<SingleResponse<CoachTripDetailResponse>> updateTripStatus(
      @PathVariable UUID id, @Valid @RequestBody UpdateCoachTripStatusRequest request) {
    CoachTripDetailResponse response = guideService.updateTripStatus(id, request);
  ...
  public ResponseEntity<SuccessResponse> markPassengerNoShow(
      @PathVariable UUID id, @PathVariable UUID bookingId) {
    guideService.markPassengerNoShow(id, bookingId);
