...
public class AdminUserServiceImpl implements AdminUserService {
    ...
    public Page<BaseUserProfileResponse> getAllUsers(UserRoles role, UserStatus status, Pageable pageable) {
    ...
    private BaseUserProfileResponse changeUserBanStatus(UUID targetUserId, UserStatus newStatus) {
        ...
        if (user.getStatus() == UserStatus.DELETED) {
            throw new BaseAppException(WebErrorCode.BAD_REQUEST, "Cannot modify status of a deleted user");
        }
    ...
    public BaseUserProfileResponse updateAvatar(UUID targetUserId, MultipartFile file) {
        ...
        if (user.getRole() == UserRoles.ADMIN || user.getRole() == UserRoles.TOURIST) {
            throw new BaseAppException(UserErrorCode.UNAUTHORIZED_ROLE); // Admin shouldn't change tourist's avatar
        }
    ...
    @Override
    @Transactional
    public BaseUserProfileResponse updateGuideProfile(UUID targetUserId, UpdateGuideProfileRequest request) {
        User user = userRepository.findById(targetUserId)
                .orElseThrow(() -> new BaseAppException(UserErrorCode.USER_NOT_FOUND));
    ...
    @Override
    @Transactional
    public BaseUserProfileResponse updateCoordinatorProfile(UUID targetUserId, UpdateCoordinatorProfileRequest request) {
        User user = userRepository.findById(targetUserId)
                .orElseThrow(() -> new BaseAppException(UserErrorCode.USER_NOT_FOUND));
    ...
    @Override
    @Transactional
    public BaseUserProfileResponse updateReceptionistProfile(UUID targetUserId, UpdateReceptionistProfileRequest request) {
        User user = userRepository.findById(targetUserId)
                .orElseThrow(() -> new BaseAppException(UserErrorCode.USER_NOT_FOUND));
