...
public interface HotelBookingDetailRepository extends JpaRepository<HotelBookingDetail, UUID> {
    ...
    @Query("SELECT DISTINCT d.hotelBooking.id, d.roomType.hotel.name "
    ...
    @Query("SELECT COUNT(d) FROM HotelBookingDetail d "
    ...
    @Query("SELECT SUM(d.quantity) FROM HotelBookingDetail d "
